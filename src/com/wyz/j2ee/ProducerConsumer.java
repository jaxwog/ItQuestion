 ...
 * <p>
 * 如果某些线程在等待某些条件触发，那当那些条件为真时，你可以用 notify 和 notifyAll 来通知那些等待在当前对象中的线程重新开始运行。
 * 如果只有一个线程在等待一个信号灯，notify和notifyAll都会通知到这个线程。但如果多个线程在等待这个信号灯，那么notify只会通知到其中一个，
 * 而其它线程并不会收到任何通知，而notifyAll会唤醒所有等待中的线程
 * <p>
 * 永远在while循环（loop）里调用 wait 和 notify，不是在 If 语句
 * if语句被catch打断，会直接跳过执行下面语句，while被打断，则会继续继续判断while语句
